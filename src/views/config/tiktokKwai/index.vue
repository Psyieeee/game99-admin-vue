<template>
  <div class="app-container">
    ...
    <el-table stripe v-loading="loading" :data="tiktokKwaiList" @selection-change="handleSelectionChange" >
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="tiktokKwaiRef" :model="form" :rules="rules" label-width="120px" style="padding-bottom: 50px">
...
<script setup name="tiktokKwaiAdjust">
import {
  addTiktokKwai,
  adjustTiktokKwaiList,
  updateTiktokKwai,
  getTiktokKwaiDetails,
  deleteTiktokKwai
} from '@/api/config/tiktokKwai.js'
...
function getList() {
  loading.value = true
  adjustTiktokKwaiList(queryParams.value).then(response => {
    tiktokKwaiList.value = response.data
    loading.value = false
  })
}
...
function handleUpdate(row) {
  ...
  getTiktokKwaiDetails(id).then(response => {
...
/** 提交按钮 submit button*/
function submitForm() {
  proxy.$refs['tiktokKwaiRef'].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateTiktokKwai(form.value).then(() => {
          proxy.$modal.msgSuccess('修改成功')
          open.value = false
          getList()
        })
      } else {
        addTiktokKwai(form.value).then(() => {
