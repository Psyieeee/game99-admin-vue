...
function getList() {
  loading.value = true;
  musicListData(queryParams.value).then(response => {
    memberMusicList.value = response.data;
...
function handleEffectChange(row) {
  ...
  proxy.$confirm('确认要' + text + '"' + row.title + '"吗?', '警告', {
    ...
    const effect = changeMusicStatus(row.id, row.effect);
    loading.value = true
    if (effect) {
      loading.value = false
      return effect
